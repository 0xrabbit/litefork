...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure)
{
    ...
    // Resolve
    const int default_port = Params().GetDefaultPort(fLFKBootstrapping);
    if (pszDest) {
    ...
    if (fLFKBootstrapping) {
        pnode->fUsesLFKMagic = false;
    }
    pnode->AddRef();
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Litefork " + FormatFullVersion();
    ...
    for (const std::string &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true))
            {
                ...
                {
                    int nOneDay = 24*3600;
                    CAddress addr = CAddress(CService(ip, Params().GetDefaultPort(fLFKBootstrapping)), requiredServiceBits);
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if ((addr.GetPort() != Params().GetDefaultPort() ||
                (fLFKBootstrapping && addr.GetPort() != Params().GetDefaultPort(true))) && nTries < 50)
                continue;
