...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-bootstrap", _("Enables Litefork bootstrap mode. Allows LFK client to connect to Bitcoin p2p network to download blockahin history."));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/litefork-project/litefork>";
    const std::string URL_WEBSITE = "<https://litefork.org>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if litefork is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if litefork is started while in a temporary directory.\n",
    ...
    fLFKBootstrapping = gArgs.GetBoolArg("-bootstrap", false);
